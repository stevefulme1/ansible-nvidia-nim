...
"""Ansible module for retrieve nim endpoint metrics."""
...
---
module: nim_metrics
short_description: Retrieve NIM endpoint metrics
description:
    - Retrieve NIM endpoint metrics using the NVIDIA REST API.
...
options:
    endpoint_id:
        description:
            - The endpoint id for the metrics.
        type: str
    state:
        description:
...
EXAMPLES = r"""
- name: Create a metrics
  stevefulme1.nvidia_nim.nim_metrics:
    endpoint_id: "example-endpoint_id"
    state: present

- name: Delete a metrics
  stevefulme1.nvidia_nim.nim_metrics:
...
RETURN = r"""
metrics:
    description: Details of the metrics.
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/metrics/{resource_id}")
...
def create_resource(module, client):
    ...
    resource = client.post("/v1/metrics", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/metrics/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/metrics/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/metrics/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/metrics/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/metrics/{existing['id']}",
...
def main():
    ...
    if existing is None:
        for req in ['endpoint_id']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a metrics.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, metrics=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, metrics=to_dict(existing))

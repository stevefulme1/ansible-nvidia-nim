...
"""Ansible module for retrieve nim model details."""
...
DOCUMENTATION = r"""
---
module: nim_model_info
short_description: Retrieve NIM model details
description:
    - Retrieve NIM model details using the NVIDIA REST API.
...
- name: Get model details
  stevefulme1.nvidia_nim.nim_model_info:
    model_id: "example-id"
"""
...
def main():
    ...
    if params.get("model_id"):
        try:
            resource = client.get(f"/v1/models/{params['model_id']}")
            module.exit_json(changed=False, model=to_dict(resource))
        except NvidiaApiError as exc:
            if exc.status == 404:
                module.exit_json(changed=False, model={})
            module.fail_json(msg=str(exc))
    else:
        try:
            result = client.get("/v1/models")
            items = result.get("data", result.get("items", []))
            module.exit_json(changed=False, models=[to_dict(i) for i in items])
        except NvidiaApiError as exc:
            module.fail_json(msg=str(exc))

...
"""Ansible module for manage nim organizations."""
...
---
module: nim_org
short_description: Manage NIM organizations
description:
    - Manage NIM organizations using the NVIDIA REST API.
...
EXAMPLES = r"""
- name: Create a org
  stevefulme1.nvidia_nim.nim_org:
    name: "example-name"
    state: present
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/orgs/{resource_id}")
...
def create_resource(module, client):
    ...
    resource = client.post("/v1/orgs", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/orgs/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/orgs/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/orgs/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/orgs/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/orgs/{existing['id']}",
...
def main():
    ...
    if params.get("org_id"):
        existing = get_resource(client, params["org_id"])
    ...
    if existing is None:
        for req in ['name']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a org.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, org=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, org=to_dict(existing))

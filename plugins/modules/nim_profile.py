...
"""Ansible module for manage nim inference profiles."""
...
---
module: nim_profile
short_description: Manage NIM inference profiles
description:
    - Manage NIM inference profiles using the NVIDIA REST API.
...
- name: Create a profile
  stevefulme1.nvidia_nim.nim_profile:
    name: "example-name"
    state: present
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/profiles/{resource_id}")
...
def create_resource(module, client):
    ...
    resource = client.post("/v1/profiles", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/profiles/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/profiles/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/profiles/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/profiles/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/profiles/{existing['id']}",
...
def main():
    ...
    if params.get("profile_id"):
        existing = get_resource(client, params["profile_id"])
    ...
    if existing is None:
        for req in ['name']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a profile.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, profile=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, profile=to_dict(existing))

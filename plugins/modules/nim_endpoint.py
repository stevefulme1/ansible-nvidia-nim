...
"""Ansible module for manage nim inference endpoints."""
...
---
module: nim_endpoint
short_description: Manage NIM inference endpoints
description:
    - Manage NIM inference endpoints using the NVIDIA REST API.
...
- name: Create a endpoint
  stevefulme1.nvidia_nim.nim_endpoint:
    name: "example-name"
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/endpoints/{resource_id}")
...
def create_resource(module, client):
    ...
    create_data = {k: v for k, v in {
        "name": params.get("name"), "model_id": params.get("model_id")
    }.items() if v is not None}
    resource = client.post("/v1/endpoints", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/endpoints/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/endpoints/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/endpoints/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/endpoints/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/endpoints/{existing['id']}",
...
def main():
    ...
    if existing is None:
        for req in ['name', 'model_id']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a endpoint.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, endpoint=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, endpoint=to_dict(existing))

...
"""Ansible module for manage nim deployments."""
...
DOCUMENTATION = r"""
---
module: nim_deployment
short_description: Manage NIM deployments
description:
    - Manage NIM deployments using the NVIDIA REST API.
...
options:
    ...
    endpoint_id:
        description:
            - The endpoint id for the deployment.
        type: str
    state:
        description:
...
- name: Create a deployment
  stevefulme1.nvidia_nim.nim_deployment:
    name: "example-name"
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/deployments/{resource_id}")
...
def create_resource(module, client):
    ...
    resource = client.post("/v1/deployments", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/deployments/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/deployments/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/deployments/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/deployments/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/deployments/{existing['id']}",
...
def main():
    ...
    if existing is None:
        for req in ['name', 'endpoint_id']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a deployment.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, deployment=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, deployment=to_dict(existing))

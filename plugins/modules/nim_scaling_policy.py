...
"""Ansible module for manage nim scaling policies."""
...
DOCUMENTATION = r"""
---
module: nim_scaling_policy
short_description: Manage NIM scaling policies
description:
    - Manage NIM scaling policies using the NVIDIA REST API.
...
options:
    policy_id:
        description:
            - The policy id for the scaling policy.
        type: str
    name:
        description:
            - The name for the scaling policy.
        type: str
    endpoint_id:
        description:
            - The endpoint id for the scaling policy.
        type: str
    state:
        description:
...
EXAMPLES = r"""
- name: Create a scaling policy
  stevefulme1.nvidia_nim.nim_scaling_policy:
    name: "example-name"
    endpoint_id: "example-endpoint_id"
    state: present

- name: Delete a scaling policy
  stevefulme1.nvidia_nim.nim_scaling_policy:
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/scaling_policies/{resource_id}")
...
def find_resource(client, name):
    ...
    if not name:
        return None
    try:
        result = client.get("/v1/scaling_policies")
...
def create_resource(module, client):
    ...
    create_data = {k: v for k, v in {
        "name": params.get("name"), "endpoint_id": params.get("endpoint_id")
    }.items() if v is not None}
    resource = client.post("/v1/scaling_policies", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/scaling_policies/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/scaling_policies/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/scaling_policies/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/scaling_policies/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/scaling_policies/{existing['id']}",
...
def main():
    ...
    if params.get("policy_id"):
        existing = get_resource(client, params["policy_id"])
    ...
    if existing is None:
        for req in ['name', 'endpoint_id']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a scaling policy.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, scaling_policy=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, scaling_policy=to_dict(existing))

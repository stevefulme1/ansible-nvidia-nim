...
"""Ansible module for run nim endpoint health checks."""
...
DOCUMENTATION = r"""
---
module: nim_health_check
short_description: Run NIM endpoint health checks
description:
    - Run NIM endpoint health checks using the NVIDIA REST API.
...
options:
    endpoint_id:
        description:
            - The endpoint id for the health check.
        type: str
    state:
        description:
...
- name: Create a health check
  stevefulme1.nvidia_nim.nim_health_check:
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/health_checks/{resource_id}")
...
def find_resource(client, name):
    ...
    if not name:
        return None
    try:
        result = client.get("/v1/health_checks")
...
def create_resource(module, client):
    ...
    resource = client.post("/v1/health_checks", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/health_checks/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/health_checks/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/health_checks/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/health_checks/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/health_checks/{existing['id']}",
...
def main():
    ...
    if existing is None:
        for req in ['endpoint_id']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a health check.")
        if module.check_mode:
            module.exit_json(changed=True)
        resource = create_resource(module, client)
        module.exit_json(changed=True, health_check=to_dict(resource))
        return
    ...
    module.exit_json(changed=False, health_check=to_dict(existing))

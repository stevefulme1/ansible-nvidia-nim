...
"""Ansible module for manage nim models."""
...
---
module: nim_model
short_description: Manage NIM models
description:
    - Manage NIM models using the NVIDIA REST API.
...
EXAMPLES = r"""
- name: Create a model
  stevefulme1.nvidia_nim.nim_model:
    name: "example-name"
    state: present
...
def get_resource(client, resource_id):
    """Get resource by ID."""
    try:
        return client.get(f"/v1/models/{resource_id}")
...
def create_resource(module, client):
    ...
    resource = client.post("/v1/models", data=create_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/models/{resource.get('id', '')}",
...
def update_resource(module, client, existing):
    ...
    resource = client.patch(f"/v1/models/{existing['id']}", data=update_data)
    if module.params.get("wait"):
        resource = wait_for_state(
            module, client,
            f"/v1/models/{existing['id']}",
            target_states=READY_STATES,
        )
    return resource
...
def delete_resource(module, client, existing):
    """Delete a resource."""
    client.delete(f"/v1/models/{existing['id']}")
    if module.params.get("wait"):
        wait_for_state(
            module, client,
            f"/v1/models/{existing['id']}",
    ...
    if existing is None:
        for req in ['name']:
            if not params.get(req):
                module.fail_json(msg=f"Parameter '{req}' is required to create a model.")
        if module.check_mode:
            module.exit_json(changed=True)
